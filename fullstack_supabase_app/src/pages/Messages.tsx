...
export default function Messages() {
  ...
  return (
    <div className="px-margin-mobile pt-4 pb-8 h-full overflow-y-auto">
      {/* Page Title & Tabs */}
      <div className="mb-6">
        <h2 className="font-headline-md text-headline-md text-on-surface mb-4">消息中心</h2>
        <div className="flex bg-surface-container-low rounded-xl p-1">
          <button onClick={() => setStatus('unread')} className={cn("flex-1 py-2 font-label-md text-label-md rounded-lg transition-colors", status === 'unread' ? 'bg-surface shadow-sm text-primary' : 'text-on-surface-variant hover:bg-surface-variant/30')}>未读消息</button>
          <button onClick={() => setStatus('archived')} className={cn("flex-1 py-2 font-label-md text-label-md rounded-lg transition-colors", status === 'archived' ? 'bg-surface shadow-sm text-primary' : 'text-on-surface-variant hover:bg-surface-variant/30')}>已存档</button>
        </div>
      </div>

      {/* Filter Chips */}
      <div className="flex gap-2 overflow-x-auto pb-4 -mx-margin-mobile px-margin-mobile hide-scrollbar">
        {['全部', '申请', '健康', '活动'].map(f => (
          <button 
            key={f}
            onClick={() => setFilter(f)}
            className={cn(
              "flex-shrink-0 px-4 py-1.5 rounded-full font-label-md text-label-md transition-colors",
              filter === f 
          ...
          <div key={item.id} className={cn("bg-surface p-4 rounded-2xl flex gap-4 shadow-[0_4px_20px_rgba(204,90,61,0.08)]", item.category === '活动' && 'border-l-4 border-secondary')}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start mb-1">
                <h3 className="font-label-md text-label-md text-on-surface truncate font-semibold">{item.title}</h3>
              ...
              <div className="flex gap-2">
                <button onClick={() => item.actionUrl && navigate(item.actionUrl)} className="px-4 py-1.5 rounded-lg bg-primary-container text-on-primary-container font-label-md text-sm hover:scale-105 transition-transform">{item.actionLabel}</button>
                {status === 'unread' && (
