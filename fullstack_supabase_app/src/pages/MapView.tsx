...
export default function MapView() {
  ...
  const [publishType, setPublishType] = useState('目击'); // Default to 目击 for this button
  ...
  return (
    <div className="absolute inset-0 z-0 h-[calc(100vh-64px-80px)] overflow-hidden">
      {showLocationToast && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 bg-surface-container-highest text-on-surface px-4 py-2 rounded-full shadow-lg text-sm font-label-md z-50 animate-in fade-in slide-in-from-top-4 duration-300">
          已定位到当前位置
        </div>
      )}
      {showHelpToast && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 bg-primary text-white px-4 py-2 rounded-full shadow-lg text-sm font-label-md z-50 animate-in fade-in slide-in-from-top-4 duration-300">
          已发送帮助意向，发起人会尽快与您联系
        </div>
      )}
      ...
      <div className="absolute inset-0 pointer-events-none mt-4">
        ...
        <div className="absolute top-4 left-0 w-full px-margin-mobile z-20 flex flex-col gap-3 pointer-events-auto">
          ...
          <div className="flex justify-center mt-2">
            <div className="bg-surface-container-high/90 backdrop-blur-sm p-1 rounded-full flex shadow-sm border border-outline-variant/30">
              <button className="px-6 py-1.5 rounded-full font-label-md text-label-md transition-all duration-200 bg-primary text-on-primary">地图视图</button>
              <button className="px-6 py-1.5 rounded-full font-label-md text-label-md transition-all duration-200 text-on-surface-variant hover:bg-surface-variant">列表视图</button>
            </div>
          </div>
        </div>
      ...
      {/* Map Controls */}
      <div className="absolute right-margin-mobile bottom-[220px] flex flex-col items-end gap-3 z-20 pointer-events-auto">
        <button 
      ...
      {/* Bottom Sheet Overlay (Recent Sightings) */}
      <div className="absolute bottom-0 left-0 w-full bg-surface-container-lowest rounded-t-[32px] shadow-[0_-8px_40px_rgba(0,0,0,0.08)] z-30 flex flex-col">
        ...
        <div className="flex gap-4 overflow-x-auto px-margin-mobile pb-6 hide-scrollbar">
      ...
      {selectedReport && (
        <div className="fixed top-0 bottom-0 left-1/2 -translate-x-1/2 w-full max-w-lg bg-background z-[100] flex flex-col shadow-2xl animate-in slide-in-from-bottom-full duration-300">
          ...
          <div className="flex-1 overflow-y-auto bg-surface pt-6 px-margin-mobile pb-10 space-y-6">
      ...
      {isPublishing && (
        <div className="fixed top-0 bottom-0 left-1/2 -translate-x-1/2 w-full max-w-lg bg-background z-[100] flex flex-col shadow-2xl animate-in fade-in slide-in-from-bottom-[50px] duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-margin-mobile space-y-6 pb-8">
            ...
            <div>
              <label className="block text-sm font-label-md text-on-surface-variant mb-2">品种或特征</label>
              <input 
                type="text" 
                placeholder="例如：中华田园猫，橘色，尾巴短" 
                className="w-full h-14 bg-surface-container-low border-transparent focus:border-primary focus:ring-0 rounded-2xl px-5 text-on-surface placeholder:text-outline-variant transition-all outline-none" 
              />
            </div>

            <div>
              <label className="block text-sm font-label-md text-on-surface-variant mb-2">事件发生时间 <span className="text-error">*</span></label>
              <input 
                type="datetime-local" 
                className="w-full h-14 bg-surface-container-low border-transparent focus:border-primary focus:ring-0 rounded-2xl px-5 text-on-surface placeholder:text-outline-variant transition-all outline-none" 
              />
            ...
            <div>
              <label className="block text-sm font-label-md text-on-surface-variant mb-2">事件发生地点 <span className="text-error">*</span></label>
              <div 
            ...
            <div>
              <label className="block text-sm font-label-md text-on-surface-variant mb-2">详情描述</label>
              <textarea 
                rows={4}
                placeholder="请提供更多的细节以帮助寻找..." 
                className="w-full bg-surface-container-low border-transparent focus:border-primary focus:ring-0 rounded-2xl p-5 text-on-surface placeholder:text-outline-variant transition-all outline-none resize-none" 
              />
            </div>

            {publishType === '丢失' && (
              <div>
                <label className="block text-sm font-label-md text-on-surface-variant mb-2">悬赏金额 (选填)</label>
                <div className="relative">
                  <span className="absolute left-5 top-1/2 -translate-y-1/2 font-bold text-on-surface">¥</span>
                  <input 
                    type="number" 
                    placeholder="输入金额" 
                    className="w-full h-14 bg-surface-container-low border-transparent focus:border-primary focus:ring-0 rounded-2xl pl-10 pr-5 text-on-surface placeholder:text-outline-variant transition-all outline-none" 
                  />
                </div>
              </div>
            )}
