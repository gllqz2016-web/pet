...
export default function EventsPage() {
  ...
  return (
    <div className="px-4 pt-4 pb-8 overflow-y-auto hidden-scrollbar">
      ...
      {featured && (
        <section className="mt-2">
          <div className="bg-surface-container rounded-[24px] overflow-hidden shadow-[0_4px_20px_rgba(204,90,61,0.08)] group">
            <div onClick={() => router.push(`/events/${featured.id}`)} className="relative h-56 w-full cursor-pointer">
            ...
            <div className="p-6">
              <h2 className="font-headline-md text-primary mb-2 font-bold">{featured.title}</h2>
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => register(featured.id)} className="bg-primary text-on-primary font-label-md px-6 py-2.5 rounded-full hover:scale-105 active:scale-95 transition-all shadow-md text-sm">
      ...
      <section className="mt-10">
        ...
        <div className="space-y-4">
          {upcoming.map((event, index) => {
            ...
            return (
              <div key={event.id} onClick={() => router.push(`/events/${event.id}`)} className="bg-surface-container-low p-4 rounded-xl flex items-center gap-4 shadow-sm border border-outline-variant/30 hover:border-primary/20 transition-colors cursor-pointer">
      ...
      <section className="mt-12 mb-4 bg-tertiary-container/10 p-8 rounded-[32px] border-2 border-dashed border-tertiary-container/30 text-center">
        ...
        <button onClick={() => router.push('/messages')} className="border-2 border-tertiary text-tertiary font-label-md px-8 py-2.5 rounded-full hover:bg-tertiary hover:text-on-tertiary transition-all duration-300 text-sm">联系运营</button>
      </section>
    </div>
  );
}
