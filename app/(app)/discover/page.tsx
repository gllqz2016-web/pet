...
export default function DiscoverPage() {
  ...
  return (
    <div className="px-4">
      {/* Top Banner */}
      <section className="-mx-4 mb-6 relative h-48 overflow-hidden rounded-b-3xl shadow-md">
        <img
      ...
      <section className="mb-8">
        ...
        <div className="flex overflow-x-auto gap-3 pb-2 scrollbar-hide hide-scrollbar -mx-4 px-4">
          {['全部', '猫咪', '狗狗'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={cn(
                "flex-shrink-0 px-6 py-2 rounded-full font-label-md shadow-md active:scale-95 transition-all",
          ...
          <button className="flex-shrink-0 px-6 py-2 rounded-full bg-secondary-fixed/30 text-on-secondary-fixed-variant font-label-md hover:bg-secondary-fixed/50 active:scale-95 transition-all flex items-center gap-1">
          ...
          <article key={pet.id} className="bg-surface rounded-3xl overflow-hidden shadow-[0_4px_20px_rgba(204,90,61,0.08)] group hover:scale-[1.02] transition-transform duration-300">
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="font-headline-md text-primary mb-1">{pet.name}</h3>
              ...
              <button
                ...
                className="w-full py-4 rounded-xl border-2 border-primary-container text-primary-container font-label-md hover:bg-primary-container/5 transition-colors active:scale-95 duration-150"
              >
                阅读故事
              </button>
            </div>
      ...
      <div className="mt-4 text-center">
        <button onClick={() => setFilter('全部')} className="font-label-md text-on-surface-variant flex items-center justify-center gap-2 mx-auto hover:text-primary transition-colors pb-8">
          查看更多伙伴
