  ...
  return (
    <div className="bg-background text-on-background font-body-md min-h-screen pb-40">
      ...
      <header className="fixed top-0 w-full z-50 shadow-[0_4px_20px_rgba(204,90,61,0.08)] bg-surface/80 backdrop-blur-md">
        <div className="flex justify-between items-center px-margin-mobile h-16 w-full max-w-lg mx-auto">
          <button onClick={() => navigate(-1)} className="text-primary hover:scale-105 transition-transform duration-200">
            <X className="w-6 h-6" />
          </button>
          <h1 className="font-headline-lg text-headline-lg-mobile text-primary tracking-tight font-bold">领养申请</h1>
          <div className="w-6"></div> {/* Spacer for symmetry */}
        </div>
      </header>

      {/* Main Content Area */}
      <main className="pt-20 px-margin-mobile max-w-lg mx-auto overflow-y-auto">
        {/* Pet Identity Header */}
        <section className="mb-10 flex items-center gap-4 bg-surface-container-low p-4 rounded-xl shadow-[0_4px_20px_rgba(204,90,61,0.08)] border border-outline-variant/30">
          ...
          <div>
            ...
            <p className="text-label-md font-label-md text-on-surface-variant uppercase tracking-wider text-xs font-semibold mt-1">最后一步：开启新的生活</p>
          </div>
        </section>
        ...
        <form className="space-y-10" onSubmit={handleSubmit} id="apply-form">
          ...
          <fieldset className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea rows={4} placeholder="请描述您的居住环境（如：是否有花园、公寓大小、是否允许养宠物等）" className="w-full p-4 bg-surface-container border-none rounded-xl focus:ring-2 focus:ring-primary-container transition-all resize-none outline-none"></textarea>
            </div>
          </fieldset>
          ...
          <fieldset className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="font-label-md text-sm font-semibold text-on-surface-variant inline-block">紧急医疗或安置计划</label>
              <textarea rows={4} placeholder={`如果发生紧急情况，您将如何安置 ${pet.name}？`} className="w-full p-4 bg-surface-container border-none rounded-xl focus:ring-2 focus:ring-primary-container transition-all resize-none outline-none"></textarea>
            </div>
          </fieldset>
          ...
          <fieldset className="space-y-6">
            ...
            <div className="p-6 bg-tertiary-fixed/30 rounded-2xl border-l-4 border-tertiary relative overflow-hidden">
              ...
              <p className="font-body-md text-on-tertiary-fixed-variant leading-relaxed text-sm">
                我郑重承诺将 {pet.name} 视为家庭成员，为其提供充足的食物、温暖的住所和必要的医疗照护。我承诺不因任何原因遗弃、转卖或虐待它。我愿意投入时间和爱心，与其建立终生的信任关系。
              </p>
            </div>
      ...
      <footer className="fixed bottom-0 left-0 w-full bg-surface/95 backdrop-blur-lg border-t border-outline-variant/20 px-margin-mobile py-6 z-50">
