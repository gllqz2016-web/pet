...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await login(authMode === 'phone' ? { phone, code } : { email, password });
      navigate('/');
    } catch (err) {
    ...
    <div className="relative min-h-screen flex flex-col items-center justify-end overflow-hidden bg-background text-on-background font-body-md">
      ...
      <div className="relative z-10 w-full px-margin-mobile pb-margin-mobile max-w-md mx-auto">
        ...
        <div className="frosted-paper rounded-[32px] p-8 shadow-[0_4px_20px_rgba(204,90,61,0.08)] w-full">
          <div className="text-center mb-8">
            <h2 className="font-headline-md text-2xl font-bold text-on-surface mb-2">加入故事</h2>
            <p className="text-on-surface-variant font-body-md text-sm">选择您的登录方式</p>
          </div>
            ...
            {authMode === 'phone' ? (
              ...
              <>
                ...
                <div className="flex justify-end pt-1">
                  <span className="text-label-md font-label-md text-primary hover:text-primary-container transition-colors text-xs cursor-pointer">忘记密码？</span>
                </div>
              </>
            )}
            
            {error && <p className="text-error text-sm text-center">{error}</p>}
            <button type="submit" disabled={submitting} className="warm-glow-gradient w-full h-14 rounded-2xl text-white font-label-md text-label-md shadow-lg active:scale-95 transition-all duration-150 mt-4 text-base tracking-wide font-bold disabled:opacity-60">
              {submitting ? '正在进入...' : '开启故事之旅'}
            </button>
          </form>
          ...
          <div className="relative my-7">
            ...
            <div className="relative flex justify-center text-label-md">
              <span className="bg-[#f2efe9] px-4 text-outline-variant text-[12px] font-semibold tracking-wider uppercase">或使用以下方式登录</span>
            </div>
          </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-body-md font-body-md text-on-surface-variant text-sm">
              还没有账号？ <button type="button" onClick={() => navigate('/register')} className="text-primary font-bold hover:underline decoration-2 underline-offset-4 cursor-pointer">立即注册</button>
            </p>
          </div>
        </div>
